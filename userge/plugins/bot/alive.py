...
if userge.has_bot:
    ...
    async def alive_cb(_, c_q: CallbackQuery):
        ...
        alive_s += f"🔥 𝚂𝚞𝚍𝚘 : {_parse_arg(Config.SUDO_ENABLED)}\n"
        alive_s += f"🔥 𝙰𝚗𝚝𝚒𝚜𝚙𝚊𝚖 : {_parse_arg(Config.ANTISPAM_SENTRY)}\n"
        alive_s += f"🔥 𝙱𝚘𝚝 𝙵𝚘𝚛𝚠𝚊𝚛𝚍𝚜 : {_parse_arg(Config.BOT_FORWARDS)}\n"
        alive_s += f"🔥 𝙿𝙼 𝙶𝚞𝚊𝚛𝚍 : {_parse_arg(not Config.ALLOW_ALL_PMS)}\n"
        alive_s += f"🔥 𝙿𝙼 𝙻𝚘𝚐𝚐𝚎𝚛 : {_parse_arg(Config.PM_LOGGING)}\n"
        alive_s += f"🔥 𝚃𝚊𝚐 𝙻𝚘𝚐𝚐𝚎𝚛 : {_parse_arg(Config.TAG_LOGGING)}"
        if allow:
            end = datetime.now()
...
"""
    ...
    def alive_buttons() -> InlineKeyboardMarkup:
        buttons = [
            [
                ...
                InlineKeyboardButton(text="👑OWNER👑", url="t.me/NeitXD"),
            ]
        ]
    ...
    def alive_default_imgs() -> str:
        alive_imgs = [
            "https://telegra.ph/file/3dd219fab3a09dbd7215f.jpg",
        ]
